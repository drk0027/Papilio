...
import Confsendmail from '../../components/admin/sistema/confsendmail'
    ...
    */
    ...
    if (isLoggedIn == true) {//si el usuario ha iniciado sesion
        if (Auth.getPerfil(token).roles == 1) {//si el usuario es administrador
            var sistema = []
            await Auth.ServerFetch(//carga la lista de campos para las configuraciones del sistema (solo lo pueden ver los administradores)
                global.server + "api/admin/ver_confsendmail",
                {
                    method: 'POST',
                    body: JSON.stringify(),
                },
            ...
            return (// panel para usuarios admin
                <Layout home login={props.isLoggedIn} token={props.token} info_sistema={props.info_sistema}>
                    <Container fluid>
                        ...
                        <Row>
                            <Col>
                                <h3>Configuraciones envio de correo electronico</h3>
                                <hr></hr>
                            </Col>
                        </Row>
                        <Row>
                            <Col>
                                <p>Aqui el administrador puede crear editar las configuraciones necesarias para el envio de notificaciones por medio de correo electronico</p>
                            </Col>
                        </Row>
                        <Container fluid>
                            <Row>
                                <Col>
                                    <Confsendmail sistema={props.sistema}></Confsendmail>
                                </Col>
                            </Row>
                        </Container>
                    </Container>
