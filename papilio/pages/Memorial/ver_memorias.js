...
import Ver_memorias from '../../components/admin/memorial/ver_memorias'
import Ver_memorias_public from '../../components/public/ver_memorias'
    ...
    */
    ...
    if (isLoggedIn == true) {//si el usuario ha iniciado sesion
        if (Auth.getPerfil(token).roles == 1) {//si el usuario es administrador
            ...
            const Columnas = ["id_usuario", "nombres", "apellidos", "fecha_nacimiento", "fecha_muerte", "slug"]
            const Donde = {}
            const Orden = [["createdAt", "DESC"]]
            const Limite = [0, 10]
            const Pagina = (0)
            const Offset = (0)
            ...
            return (// panel para usuarios admin
                <Layout home login={props.isLoggedIn} token={props.token} info_sistema={props.info_sistema}>
                    <Container fluid>
                        ...
                        <Row>
                            <Col>
                                <h3>Vista de memorias registradas</h3>
                                <hr></hr>
                            </Col>
                        </Row>
                        <Row>
                            <Col>
                                <p>Aqui el administrador puede crear editar o eliminar entradas del servicio memorial</p>
                            </Col>
                        </Row>
                        <Container fluid>
                            <Row>
                                <Col>
                                    <Ver_memorias sistema={props.sistema} />
                                </Col>
                            </Row>
                        </Container>
                    </Container>
        ...
        if (Auth.getPerfil(props.token).roles == 2) {  //panel para usuarios normales
            return (
                <Layout home login={props.isLoggedIn} token={props.token} info_sistema={props.info_sistema}>
                    <Container fluid>
                        <h3>En Memoria de...</h3>
                        <Ver_memorias_public sistema={props.sistema}></Ver_memorias_public>
                    </Container>
                </Layout>
            )
        }
