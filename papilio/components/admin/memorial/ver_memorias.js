...
import Editar_memoria from "./editar_memoria"
...
export default function inicio(props) {
    ...
    const [Columnas, SetColumnas] = useState(["id_memoria", "nombres", "apellidos","contenido", "contenido","fecha_nacimiento","slug","fecha_muerte","createdAt"])
    ...
    function eliminar(e) {
        ...
        console.log(Tabla.contenido[e.currentTarget.name].id_memoria) // obtiene el id de la entrada segund el indice seleccionado
        //llamar a la api para borrar el registro
        var donde = {
            id_memoria: Tabla.contenido[e.currentTarget.name].id_memoria
        }
        Auth.fetch(//carga la lista de campos para las configuraciones del sistema (solo lo pueden ver los administradores)
            global.server + "api/admin/eliminar_memoria",
            {
                method: 'POST',
                body: JSON.stringify({
                    donde,
    ...
    function carga_estado() {
        Auth.fetch(
            global.server + "api/admin/ver_memorias",
            {
                method: 'POST',
                body: JSON.stringify({
                    Columnas,
            ...
            <Row>
                            ...
                            <tbody>
                                {Tabla.contenido.map((fila, index) => (
                                    <tr >
                                        ...
                                        <td >{fila.usuario.nombres}</td>
                                        <td >{fila.usuario.apellidos}</td>
                                        <td >{fila.usuario.cedula}</td>
                ...
                <Modal show={Show} onHide={handleClose}>
                    <Modal.Header closeButton>
                        <Modal.Title>
                            <h3>Editar Memoria de {Fila.nombres + " " + Fila.apellidos}</h3>
                            </Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                        <Editar_memoria Fila={Fila}></Editar_memoria>
                    </Modal.Body>
                    <Modal.Footer>
